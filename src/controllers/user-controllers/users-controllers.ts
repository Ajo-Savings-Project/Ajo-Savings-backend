...
export const resendOTP = async (req: JwtPayload, res: Response) => {
  ...
  // Find the user by id
  const user = await Users.findOne({ where: {id: user_id } }) as unknown as UserAttributes;
...
export const getAllUserTransactionsByUser = async (
  ...
  try {
    ...
    if (user_Id) {
      const allUserTransactions = await Transactions.findAll({
        where: { owner_id: user_Id },
      });

      if (allUserTransactions) {
        allUserTransactions.sort((a:any,b:any)=>b.created_at-a.created_at)
        return res.status(200).json({
...
export const getUserUpcomingActivities = async (
  ...
  try {
    ...
    const groupsUserBelongs = await Groups.findAll({
      where: {
        [Op.or]: [
          { admin_id: userId },
    ...
    if (!groupsUserBelongs || groupsUserBelongs.length === 0) {
      return res
        .status(404)
        .json({ message: `Error getting the groups user belongs to` });
    }
...
export const getGlobalWallet = async (req:JwtPayload, res:Response) => {
  try{
    const userId = req.user.id
    const globalWallet = await Wallets.findOne({where: {user_id: userId, type: type.GLOBAL}})
    if(!globalWallet) return res.status(404).json({message: `Wallet not found`})
    return res.status(200).json({message: `Wallet fetched successfully`, data: globalWallet})
  }catch(err:any){
    console.log(err.message)
    return res.status(500).json({
      message: `Internal Server Error`
