...
export const addMoneytoTotalGroupSavings = async (req: JwtPayload, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    const owner = await Users.findOne({where: {id:userId}}) as unknown as UserAttributes
    ...
    if (globalWallet.total_amount < amount) {
      ...
      const failedTransaction = await Transactions.create({
        ...
        created_at: new Date(),
        receiver: `${owner.firstName} ${owner.lastName}`, // Provide a default value for created_at
      });
    ...
    // Create a successful transaction record
        const successfulTransaction = await Transactions.create({
            ...
            receiver: `${owner.firstName} ${owner.lastName}`, // Add the receiver (you might need to replace this with the actual receiver)
          });
