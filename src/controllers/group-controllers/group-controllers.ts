...
export const userGetSingleGroup = async (req: JwtPayload, res: Response) => {
  try {
    ...
    const group = (await Groups.findOne({ where: { id: groupId } })) as unknown as GroupAttributes
...
export const getAllMemberGroups = async (req: JwtPayload, res: Response) => {
  try {
    ...
    const allMemberGroups = allGroups.filter((group) => group.members.some((member: any)  => member.member_id === userId ) )
...
export const getSingleGroupTransactions = async (req: JwtPayload, res: Response, next: NextFunction) => {
  try {
    const groupId = req.params.id

    const allGroupTransactions = await Transactions.findAll({where: {owner_id: groupId}})
