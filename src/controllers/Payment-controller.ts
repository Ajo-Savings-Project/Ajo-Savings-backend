  ...
  try {
    ...
    if (response.status === true && response.data.status === "success") {
      ...
      try {
        ...
        const userWallet = await Wallets.findOne({where : {user_id: owner_id, type:'Global'}}) as unknown as UserAttributes

        userWallet.total_amount += amount

        await Wallets.update(
          {
            total_amount: userWallet.total_amount
        },
